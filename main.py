...
keranjang_img = cv2.resize(keranjang_img, (150, 150))  # Ukuran konstan

# Setting bola
ball_radius = 30
colors = [(0, 0, 255), (0, 255, 255), (0, 255, 0), (255, 0, 0)]  # Termasuk biru di akhir
...
while cap.isOpened():
    ...
    if nose_tip_x is not None and nose_tip_y is not None:
        ...
        if keranjang_img.shape[2] == 4:
            ...
            for c in range(3):
                roi[:, :, c] = (keranjang_alpha * keranjang_rgb[:, :, c] + (1 - keranjang_alpha) * roi[:, :, c])
        else:
    ...
    if not game_over:
        for ball in balls[:]:
            if not ball["active"]:
                if current_time >= ball["spawn_time"]:
                    ball["active"] = True
                else:
                    continue
            ...
            if nose_tip_x is not None and nose_tip_y is not None:
                ...
                if dx * dx + dy * dy < ball_radius ** 2:
                    if ball["color"] == (255, 0, 0):  # Biru (BGR)
                        score += 1
                        balls.remove(ball)
                        balls.append(create_ball())
                        continue
    ...
    cv2.putText(frame, f"Waktu: {elapsed_time}s", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    if game_over:
        cv2.putText(frame, "GAME OVER", (iw//2 - 150, ih//2), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
